...
void compare_highest_grade(Classes *c, int class_id, double grade) {
    double curr_highest_grade = c->highest_grades[class_id];
    if (grade > curr_highest_grade) {
        c->highest_grades[class_id] = grade;
    }
}

double get_highest_grade_per_class(Classes *c, int class_id) {
    return c->highest_grades[class_id];
}
...
void process_grades(Classes *c, int class_id) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        compare_highest_grade(c, class_id, grade);
...
void registry_to_string(Classes *c, int class_id) {
    ...
    double highest_grade = get_highest_grade_per_class(c, class_id);
    printf(">>> Highest grade for Class %d: %.2f\n", class_id, highest_grade);
}
