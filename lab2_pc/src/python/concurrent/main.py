...
class Classes:
    ...
    def __init__(self, num_classes: int, num_students_per_class: int) -> None:
        ...
        self.highest_grade_per_class: List[float] = list(range(1, num_classes + 1))
    ...
    def _compare_highest_grade(self, grade: str, class_id: int):
        curr_highest_grade = self.highest_grade_per_class[class_id - 1]
        if float(grade) > curr_highest_grade:
            self.highest_grade_per_class[class_id - 1] = float(grade)
    ...
    def get_highest_grade_in_class(self, class_id: int) -> float:
        return self.highest_grade_per_class[class_id - 1]
    ...
    def process_grades(self, class_id: int) -> None:
        ...
        for student_id in their_alumni:
            ...
            self._compare_highest_grade(grade, class_id)
    ...
    def registry_to_string(self, class_id: int) -> None:
        ...
        print(f">>> Highest grade for Class {class_id}: {self.get_highest_grade_in_class(class_id)}")
